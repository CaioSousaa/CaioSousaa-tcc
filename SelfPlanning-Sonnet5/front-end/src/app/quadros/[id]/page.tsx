...
import { Board, BOARD_COLOR_CLASSES } from "@/lib/board-colors";
...
function BoardDetailContent() {
  ...
  async function loadBoard() {
    setLoading(true);
    try {
      const [boardResponse, listsResponse, labelsResponse] = await Promise.all([
  ...
  return (
    <div className="flex min-h-screen flex-col bg-zinc-50">
      <header className="flex items-center gap-4 border-b border-zinc-200 bg-white px-8 py-4">
        <Link href="/quadros" className="text-sm text-slate-500 hover:text-slate-700">
          ← Quadros
        </Link>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-200 bg-white px-8 py-3">
          {boardLabels.length > 0 ? (
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-sm text-slate-500">Filtrar por etiqueta:</span>
              <button
        ...
        <div className="flex items-start gap-4">
          {lists.map((list, index) => (
            <ListColumn
              ...
              onMoveLeft={() => moveList(index, -1)}
              onMoveRight={() => moveList(index, 1)}
          ...
          <div className="w-72 shrink-0">
            {showAddForm ? (
              ...
              <button
                ...
                className="w-full rounded-lg border border-dashed border-zinc-300 px-3 py-3 text-left text-sm text-slate-500 hover:border-slate-400 hover:text-slate-700"
              >
                + Adicionar lista
              </button>
            )}
