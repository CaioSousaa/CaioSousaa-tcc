    ...
    <div className="mb-6">
        ...
        {items.map((item) => (
          <div key={item.id} className="group flex items-center gap-2">
            ...
            <button
              ...
              className="rounded px-1 text-xs text-slate-400 opacity-0 hover:bg-red-50 hover:text-red-600 group-hover:opacity-100"
            >
              ✕
            </button>
          </div>
      ...
      <form onSubmit={handleAddItem} className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={adding}
          className="shrink-0 rounded-md bg-slate-800 px-3 py-1.5 text-sm font-medium text-white hover:bg-slate-900 disabled:opacity-60"
        >
          Adicionar
        </button>
      </form>
